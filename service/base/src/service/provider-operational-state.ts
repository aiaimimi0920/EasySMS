...
export function classifySmsProviderRouteFailure(error: unknown): SmsProviderRouteFailureDecision {
  ...
  if (isTransientNetworkConnectionFailure(message)) {
    return {
      ...
      escalateAfterCount: Number.POSITIVE_INFINITY,
    };
  }
...
export class EasySmsProviderOperationalState {
  ...
  public snapshot(now: Date = new Date()): EasySmsRuntimeStateSnapshot {
    ...
    return {
      ...
      probeHistory: this.listProbeHistory({}, now),
  ...
  public listProbeHistory(filters: SmsProviderProbeHistoryQueryFilters = {}, now: Date = new Date()): SmsProviderProbeHistoryEntry[] {
    this.refresh(now);
    const output: SmsProviderProbeHistoryEntry[] = [];

    for (const [key, entries] of this.probeHistoryByProvider.entries()) {
      if (filters.providerKey && key !== filters.providerKey) {
        continue;
      }
    ...
    if (filters.routeKind) {
      filtered = filtered.filter((entry) => entry.routeKind === filters.routeKind);
    }
    if (filters.healthState) {
      filtered = filtered.filter((entry) => entry.healthState === filters.healthState);
    }
    if (filters.since) {
      filtered = filtered.filter((entry) => Date.parse(entry.checkedAt) >= filters.since!.getTime());
    }
    if (filters.until) {
      filtered = filtered.filter((entry) => Date.parse(entry.checkedAt) <= filters.until!.getTime());
    }

    const newestFirst = filters.newestFirst !== false;
    filtered.sort((left, right) => newestFirst
