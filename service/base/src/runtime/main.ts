...
async function main(): Promise<void> {
  ...
  try {
    const loadedState = await loadEasySmsRuntimeState(config);
    service.hydrateRuntimeState(loadedState);
    service.recordRuntimeStateLoad({
      status: !config.persistence.enabled
        ? "skipped"
        : loadedState
          ? "loaded"
          : "empty",
      detail: !config.persistence.enabled
        ? "Persistence is disabled."
        : loadedState
          ? "Runtime state was loaded from the persistence store."
          : "No persisted runtime state was found.",
    });
  } catch (error) {
    service.recordRuntimeStateLoad({
      status: "failed",
      detail: "Failed to load runtime state from the persistence store.",
      error,
    });
    throw error;
  }
